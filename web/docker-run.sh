...
function waitForDataBase()
{
  r=1
  while [[ ${r} -ne 0 ]]; do
    mysql -u ${DOLIBARR_DB_USER} --protocol tcp -p${DOLIBARR_DB_PASSWORD} -h ${DOLIBARR_DB_HOST} -e "status" > /dev/null 2>&1
    r=$?
    if [[ ${r} -ne 0 ]]; then
...
function initializeDatabase()
{
  ...
  mysql -u ${DOLIBARR_DB_USER} -p${DOLIBARR_DB_PASSWORD} -h ${DOLIBARR_DB_HOST} ${DOLIBARR_DB_NAME} -e "DELETE FROM llx_const WHERE name='MAIN_LANG_DEFAULT';" > /dev/null 2>&1
  mysql -u ${DOLIBARR_DB_USER} -p${DOLIBARR_DB_PASSWORD} -h ${DOLIBARR_DB_HOST} ${DOLIBARR_DB_NAME} -e "INSERT INTO llx_const(name,value,type,visible,note,entity) values('MAIN_VERSION_LAST_INSTALL', '${DOLIBARR_VERSION}', 'chaine', 0, 'Dolibarr version when install', 0);" > /dev/null 2>&1
  mysql -u ${DOLIBARR_DB_USER} -p${DOLIBARR_DB_PASSWORD} -h ${DOLIBARR_DB_HOST} ${DOLIBARR_DB_NAME} -e "INSERT INTO llx_const(name,value,type,visible,note,entity) VALUES ('MAIN_LANG_DEFAULT', 'auto', 'chaine', 0, 'Default language', 1);" > /dev/null 2>&1
}
...
function migrateDatabase()
{
  ...
  mysqldump -u ${DOLIBARR_DB_USER} -p${DOLIBARR_DB_PASSWORD} -h ${DOLIBARR_DB_HOST} ${DOLIBARR_DB_NAME} > /var/www/documents/dump.sql
  r=${?}
  if [[ ${r} -ne 0 ]]; then
  ...
  if [[ ${r} -ne 0 ]]; then
    ...
    mysql -u ${DOLIBARR_DB_USER} -p${DOLIBARR_DB_PASSWORD} -h ${DOLIBARR_DB_HOST} ${DOLIBARR_DB_NAME} < /var/www/documents/dump.sql
    echo "DB Restored ..."
    return ${r}
  else
